...
abstract contract HederaTokenService is HederaResponseCodes {
    ...
    /// Withdraws hbar to the calling account/contract,
    /// where the amount is the value needed to zero balance the transfers. Regular signing rules apply for sending
    /// (positive amount) or receiving (negative amount)
    function transferHbar(address accountId, int64 amount) internal returns (int responseCode) 
    {
        (bool success, bytes memory result) = precompileAddress.call(
            abi.encodeWithSelector(IHederaTokenService.transferHbar.selector,
            accountId, amount));
        responseCode = success ? abi.decode(result, (int32)) : HederaResponseCodes.UNKNOWN;
    }
